...
## Batch 1 — wire + WPF セル読取（ブランチ: `phase9/batch-1-get-cell-text`）

- [ ] Protocol: `getCellText`（params: automationId, row, column → text）
- [ ] WPF: 行 realize → Text 列セルの表示文字列を返す
- [ ] Core: `GetCellTextAsync(row, column)`
- [ ] ホスト側ユニット／薄い検証
...
## Batch 2 — セル書込（ブランチ: `phase9/batch-2-set-cell-value`）

- [ ] Protocol: `setCellValue`（params: automationId, row, column, value）
- [ ] WPF: BeginEdit → 編集要素へ値 → CommitEdit（読取専用列は `action.failed`）
- [ ] Core: `SetCellValueAsync(row, column, value)`
- [ ] Sample: `SampleGrid` を編集可能 Text 列に（FullRow/Single 維持）
...
## Batch 3 — Expect + Sample E2E（ブランチ: `phase9/batch-3-expect-e2e`）

- [ ] `ExpectCellTextAsync(row, column, expected)`
- [ ] Sample Fluent E2E: set → expect（必要なら get）
- [ ] `graft-core.md` にセル R/W の短いメモ
...
## Batch 4 — Scenario / MCP + docs（ブランチ: `phase9/batch-4-docs`）

- [ ] scenario schema: `getCellText` / `setCellValue` / `expectCellText`
- [ ] ScenarioJson / Runner / MCP 原子ツール
- [ ] Scenario E2E（薄い追従）
- [ ] `project.md` / 本ファイル完了チェック。次フェーズメモ: **OS 共通ダイアログ方針**
...
- [ ] `GetCellTextAsync` / `SetCellValueAsync`（ホスト＋row/column）がある
- [ ] Text 列で BeginEdit/CommitEdit 書込ができる
- [ ] `ExpectCellTextAsync` がある
- [ ] Sample E2E（FullRow/Single + 編集可能 Text）が緑
- [ ] Scenario / MCP から同操作が呼べる（薄い追従）
- [ ] 列キー / CheckBox・Template 列 / Cell 選択 / ツリー DataGridCell / OS ダイアログ / Avalonia / Inspector は **含めない**
