...
- Scenario JSON: `ScenarioJson.ParseFile` → `ScenarioRunner.RunAsync`（`launch` / `invoke` / `setValue` / `toggle` / `sendKeys` / `scrollIntoView` / `select` / `expand` / `collapse` / `expectName`）。契約は `.dev/scenario.schema.json`
