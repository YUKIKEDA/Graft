...
- Scenario JSON: `ScenarioJson.ParseFile` → `ScenarioRunner.RunAsync`（上記に加え `armOpenFile` / `armSaveFile` / `armOpenFolder` 系 / セル・窓系）。契約は `.dev/scenario.schema.json`。例: `tests/sample-apps/SampleWpfApp.Tests/Scenarios/`
- MCP: `Graft.McpServer`（stdio）。原子ツールに `graft_arm_open_file` / `graft_arm_save_file` / `graft_arm_open_folder` 系とセル・窓系を含む。失敗時は `IsError` + FailureReport JSON
- invoke / setValue はネイティブ → Peer → SendInput フォールバック（クリック / クリア+タイプ）
- 未実装（後続）: MessageBox シーム、DataGrid 列キー／他列種、Avalonia、Inspector、ファジー自己修復、シナリオ自動書き換え、`typeHuman` / chord DSL、複数選択
