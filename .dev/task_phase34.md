...
受け入れ条件（要約）: 実ユースケース寄りの Todo サンプル（MVVM/DI/テーマ/実 JSON/詳細窓/Import・Export）とストーリー E2E（1 本・フィルタ／テーマ／CRUD 含む）を追加し、`graft-core.md` の正本例を差し替える。  
...
| E2E | ストーリー 1 本（保存先→追加→Import→検索／優先度／状態フィルタ→テーマ→チェック選択→編集→削除→Export） |
