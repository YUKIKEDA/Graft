...
public sealed class WpfUiCaptureTests
{
    ...
    public void GetTreeScreenshotResolveInvokeAndSetValue_OnShownWindow_Succeed()
    {
        ...
        try
        {
            ...
            Assert.True(capture.PngBytes.AsSpan(0, PngSignature.Length).SequenceEqual(PngSignature), "Expected PNG signature on captured bytes.");

            var buttonClip = screenshotProvider.Capture(new ScreenshotOptions { Selector = new ElementSelector { AutomationId = "SampleButton" } });
            Assert.Equal("png", buttonClip.Meta.Format);
            Assert.True(buttonClip.Meta.Width > 0);
            Assert.True(buttonClip.Meta.Height > 0);
            Assert.True(
                buttonClip.Meta.Width < capture.Meta.Width || buttonClip.Meta.Height < capture.Meta.Height,
                "Element clip should be smaller than the window screenshot."
            );
            Assert.True(buttonClip.PngBytes.AsSpan(0, PngSignature.Length).SequenceEqual(PngSignature), "Expected PNG signature on element clip.");
            ...
            var emptyClip = Assert.Throws<ElementActionException>(() =>
                screenshotProvider.Capture(new ScreenshotOptions { Selector = new ElementSelector { AutomationId = "CollapsedShotButton" } })
            );
            ...
            var openerClosed = screenshotProvider.Capture(
                new ScreenshotOptions { Selector = new ElementSelector { AutomationId = "PopupShotOpener" } }
            );
            popup.IsOpen = true;
            window.UpdateLayout();
            ...
            Assert.True(
                openerClip.Meta.Width > openerClosed.Meta.Width || openerClip.Meta.Height > openerClosed.Meta.Height,
                "Open Popup targeting the opener should be composited into the opener clip."
            );
            var popupClip = screenshotProvider.Capture(new ScreenshotOptions { Selector = new ElementSelector { AutomationId = "PopupShotButton" } });
            Assert.True(popupClip.Meta.Width > 0);
            Assert.True(popupClip.Meta.Height > 0);
            Assert.True(popupClip.PngBytes.AsSpan(0, PngSignature.Length).SequenceEqual(PngSignature), "Expected PNG signature on popup clip.");
            ...
            var tipClip = screenshotProvider.Capture(new ScreenshotOptions { Selector = new ElementSelector { RuntimeId = tipNode.RuntimeId } });
            Assert.True(tipClip.Meta.Width > 0);
            Assert.True(tipClip.Meta.Height > 0);
            Assert.True(tipClip.PngBytes.AsSpan(0, PngSignature.Length).SequenceEqual(PngSignature), "Expected PNG signature on tooltip clip.");
            var sectionClip = screenshotProvider.Capture(
                new ScreenshotOptions { Selector = new ElementSelector { AutomationId = "TipShotSection" } }
            );
            Assert.True(sectionClip.Meta.Width > 0);
            Assert.True(sectionClip.Meta.Height > 0);
            Assert.True(
                sectionClip.Meta.Width > tipClip.Meta.Width || sectionClip.Meta.Height > tipClip.Meta.Height,
            ...
            var afterById = screenshotProvider.Capture(new ScreenshotOptions { Selector = new ElementSelector { AutomationId = "AfterOpenTip" } });
            var afterByRuntime = screenshotProvider.Capture(
                new ScreenshotOptions { Selector = new ElementSelector { RuntimeId = afterTipNode.RuntimeId } }
            );
            ...
            var hostOnly = screenshotProvider.Capture(
                new ScreenshotOptions { Selector = new ElementSelector { AutomationId = "ContextMenuShotHost" } }
            );
            ...
            Assert.True(
                hostWithMenu.Meta.Width > hostOnly.Meta.Width || hostWithMenu.Meta.Height > hostOnly.Meta.Height,
            ...
            var notFound = Assert.Throws<ElementResolveException>(() => resolver.Resolve(new ElementSelector { AutomationId = "DoesNotExist" }));
            ...
            var mouseResolved = resolver.Resolve(new ElementSelector { AutomationId = "SampleMouseTarget" });
